 ...
 * @type {*[]}
 */
const httpProxyWebOutgoingPasses = Object.keys(httpProxyWebOutgoing).map(function(pass) {
    return httpProxyWebOutgoing[pass];
});
...
function handleResponseWithHttpProxy(proxyRes, req, res, options) {
    ...
    if(!res.headersSent) {
        for (let i = 0; i < httpProxyWebOutgoingPasses.length; i++) {
